...
import Lukasiewicz.language.Lukasiewicz.LukasiewiczLexer;
import Lukasiewicz.language.Lukasiewicz.LukasiewiczParser;
import Lukasiewicz.language.Lukasiewicz.LukasiewiczProblemAnalyzer;
import Lukasiewicz.language.Lukasiewicz.LukasiewiczTableauObjectFactory;

import static mettel.util.MettelStrings.FILE_SEPARATOR;
public class LukasiewiczBenchmark{
    ...
    private static MettelProverThread createAndStartNewThread(String problemFile) throws IOException, RecognitionException, InterruptedException{
        for(int i = 0; i < MAX_THREAD_TRIES; i++){
            try{
                ...
                LukasiewiczTableauObjectFactory tfactory = new LukasiewiczTableauObjectFactory();
    ...
    private static void updateProblemFile(MettelProverThread thread) throws IOException, RecognitionException{
        ...
        if(!pFile.existsStatisticsProperties()){
            LukasiewiczProblemAnalyzer pAnalyzer = new LukasiewiczProblemAnalyzer(problemPath);
            pStat = pAnalyzer.getStatistics();
            pFile.setStatisticsProperties(pStat);
        }else{
            pStat = pFile.getStatisticsProperties();
        }
        pFile.addStatusProperty("LukasiewiczProver", thread.result() == null? CSV_NOT_RELEVANT_FIELD: (thread.result()? "Satisfiable":"Unsatisfiable"));
        pFile.writeToFile();
    }
    ...
    private static void fillCsvStatistics(String[] problemInfo){
        problemInfo[10] = pStat.getProperty("valuation.total.variable.number");
        problemInfo[11] = pStat.getProperty("valuation.total.connective.number");
        problemInfo[12] = pStat.getProperty("valuation.total.variable.occurences");
        problemInfo[13] = pStat.getProperty("valuation.total.variable.max-depth");
        problemInfo[14] = pStat.getProperty("valuation.total.connective.occurences");
        problemInfo[15] = pStat.getProperty("valuation.total.connective.max-depth");
        problemInfo[16] = pStat.getProperty("valuation.true.occurences");
        problemInfo[17] = pStat.getProperty("valuation.true.max-depth");
        problemInfo[18] = pStat.getProperty("valuation.unknown.occurences");
        problemInfo[19] = pStat.getProperty("valuation.unknown.max-depth");
        problemInfo[20] = pStat.getProperty("valuation.false.occurences");
        problemInfo[21] = pStat.getProperty("valuation.false.max-depth");
        problemInfo[22] = pStat.getProperty("formula.total.variable.number");
        problemInfo[23] = pStat.getProperty("formula.total.connective.number");
        problemInfo[24] = pStat.getProperty("formula.total.variable.occurences");
        problemInfo[25] = pStat.getProperty("formula.total.variable.max-depth");
        problemInfo[26] = pStat.getProperty("formula.total.connective.occurences");
        problemInfo[27] = pStat.getProperty("formula.total.connective.max-depth");
        problemInfo[28] = pStat.getProperty("formula.negation.occurences");
        problemInfo[29] = pStat.getProperty("formula.negation.max-depth");
        problemInfo[30] = pStat.getProperty("formula.conjunction.occurences");
        problemInfo[31] = pStat.getProperty("formula.conjunction.max-depth");
        problemInfo[32] = pStat.getProperty("formula.disjunction.occurences");
        problemInfo[33] = pStat.getProperty("formula.disjunction.max-depth");
        problemInfo[34] = pStat.getProperty("formula.implication.occurences");
        problemInfo[35] = pStat.getProperty("formula.implication.max-depth");
        problemInfo[36] = pStat.getProperty("total.variable.occurences");
        problemInfo[37] = pStat.getProperty("total.variable.max-depth");
        problemInfo[38] = pStat.getProperty("total.variable.number");
        problemInfo[39] = pStat.getProperty("total.connective.occurences");
        problemInfo[40] = pStat.getProperty("total.connective.max-depth");
        problemInfo[41] = pStat.getProperty("total.connective.number");
        problemInfo[42] = pStat.getProperty("total.symbol.max-depth");
        problemInfo[43] = pStat.getProperty("total.symbol.length");
    }
    ...
    public static void parseCalculus(Set<MettelGeneralTableauRule> calculus) throws IOException, RecognitionException{
        CharStream tin = new ANTLRInputStream(LukasiewiczBenchmark.class.getResourceAsStream("calculus"));
