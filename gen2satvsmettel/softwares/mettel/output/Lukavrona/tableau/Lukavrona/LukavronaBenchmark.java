 ...
 */
package Lukavrona.tableau.Lukavrona;
...
import Lukavrona.language.Lukavrona.LukavronaLexer;
import Lukavrona.language.Lukavrona.LukavronaParser;
import Lukavrona.language.Lukavrona.LukavronaProblemAnalyzer;
import Lukavrona.language.Lukavrona.LukavronaTableauObjectFactory;

import static mettel.util.MettelStrings.FILE_SEPARATOR;
public class LukavronaBenchmark{
    ...
    final private static LukavronaParser parser = new LukavronaParser(tokens);
    ...
    public static void main(String[] args) throws IOException{
        try{
            ...
            File csvFile = new File(inDir + FILE_SEPARATOR + "Lukavrona.csv");
    ...
    private static MettelProverThread createAndStartNewThread(String problemFile) throws IOException, RecognitionException, InterruptedException{
        for(int i = 0; i < MAX_THREAD_TRIES; i++){
            try{
                ...
                LukavronaTableauObjectFactory tfactory = new LukavronaTableauObjectFactory();
    ...
    private static void updateProblemFile(MettelProverThread thread) throws IOException, RecognitionException{
        ...
        if(!pFile.existsStatisticsProperties()){
            LukavronaProblemAnalyzer pAnalyzer = new LukavronaProblemAnalyzer(problemPath);
            pStat = pAnalyzer.getStatistics();
            pFile.setStatisticsProperties(pStat);
        }else{
            pStat = pFile.getStatisticsProperties();
        }
        pFile.addStatusProperty("LukavronaProver", thread.result() == null? CSV_NOT_RELEVANT_FIELD: (thread.result()? "Satisfiable":"Unsatisfiable"));
        pFile.writeToFile();
    }
    ...
    public static void parseCalculus(Set<MettelGeneralTableauRule> calculus) throws IOException, RecognitionException{
        CharStream tin = new ANTLRInputStream(LukavronaBenchmark.class.getResourceAsStream("calculus"));
        tokens.setTokenSource(new LukavronaLexer(tin));
