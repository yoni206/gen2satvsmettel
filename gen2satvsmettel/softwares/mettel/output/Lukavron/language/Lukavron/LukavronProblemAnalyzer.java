...
public class LukavronProblemAnalyzer{
    ...
    public LukavronProblemAnalyzer(String problemFile) throws IOException, RecognitionException{
        this(new FileInputStream(problemFile));
    }

    public LukavronProblemAnalyzer(InputStream problemReader) throws IOException, RecognitionException{
        ...
        ArrayList<LukavronExpression> list = parser.expressions();

        for (LukavronExpression expression : list){
    ...
    private void determineSymbol(LukavronExpression expression){
        currentDepth++;
        if (expression instanceof Lukavron_valuation){
            if (expression instanceof Lukavron_valuationVariable){
                Integer value;
                String variableName = ((Lukavron_valuationVariable)expression).toString();
            ...
            }else if (expression instanceof Lukavron_true_valuation){
                update_valuationConnectiveOccurences("true");
                update_valuationConnectiveMaxDepth("true");
                determineSymbol(((Lukavron_true_valuation)expression).e0);
            }else if (expression instanceof Lukavron_false_valuation){
                update_valuationConnectiveOccurences("false");
                update_valuationConnectiveMaxDepth("false");
                determineSymbol(((Lukavron_false_valuation)expression).e0);
            }
        }else         if (expression instanceof Lukavron_formula){
            if (expression instanceof Lukavron_formulaVariable){
                Integer value;
                String variableName = ((Lukavron_formulaVariable)expression).toString();
            ...
            }else if (expression instanceof Lukavron_negation_formula){
                update_formulaConnectiveOccurences("negation");
                update_formulaConnectiveMaxDepth("negation");
                determineSymbol(((Lukavron_negation_formula)expression).e0);
            }else if (expression instanceof Lukavron_conjunction_formula){
                update_formulaConnectiveOccurences("conjunction");
                update_formulaConnectiveMaxDepth("conjunction");
                determineSymbol(((Lukavron_conjunction_formula)expression).e0);
                determineSymbol(((Lukavron_conjunction_formula)expression).e1);
            }else if (expression instanceof Lukavron_disjunction_formula){
                update_formulaConnectiveOccurences("disjunction");
                update_formulaConnectiveMaxDepth("disjunction");
                determineSymbol(((Lukavron_disjunction_formula)expression).e0);
                determineSymbol(((Lukavron_disjunction_formula)expression).e1);
            }else if (expression instanceof Lukavron_implication_formula){
                update_formulaConnectiveOccurences("implication");
                update_formulaConnectiveMaxDepth("implication");
                determineSymbol(((Lukavron_implication_formula)expression).e0);
                determineSymbol(((Lukavron_implication_formula)expression).e1);
            }
        }
        currentDepth--;
    }
