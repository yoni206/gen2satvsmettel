 ...
 */
package Lukavron.tableau.Lukavron;
...
import Lukavron.language.Lukavron.LukavronLexer;
import Lukavron.language.Lukavron.LukavronParser;
import Lukavron.language.Lukavron.LukavronProblemAnalyzer;
import Lukavron.language.Lukavron.LukavronTableauObjectFactory;

import static mettel.util.MettelStrings.FILE_SEPARATOR;
public class LukavronBenchmark{
    ...
    final private static LukavronParser parser = new LukavronParser(tokens);
    ...
    public static void main(String[] args) throws IOException{
        try{
            ...
            File csvFile = new File(inDir + FILE_SEPARATOR + "Lukavron.csv");
    ...
    private static MettelProverThread createAndStartNewThread(String problemFile) throws IOException, RecognitionException, InterruptedException{
        for(int i = 0; i < MAX_THREAD_TRIES; i++){
            try{
                ...
                tokens.setTokenSource(new LukavronLexer(in));
                ArrayList<Lukavron_valuation> list = new ArrayList<Lukavron_valuation>();
                parser.valuations(list);
                LukavronTableauObjectFactory tfactory = new LukavronTableauObjectFactory();
    ...
    private static void updateProblemFile(MettelProverThread thread) throws IOException, RecognitionException{
        ...
        if(!pFile.existsStatisticsProperties()){
            LukavronProblemAnalyzer pAnalyzer = new LukavronProblemAnalyzer(problemPath);
            pStat = pAnalyzer.getStatistics();
            pFile.setStatisticsProperties(pStat);
        }else{
            pStat = pFile.getStatisticsProperties();
        }
        pFile.addStatusProperty("LukavronProver", thread.result() == null? CSV_NOT_RELEVANT_FIELD: (thread.result()? "Satisfiable":"Unsatisfiable"));
        pFile.writeToFile();
    }
    ...
    private static void fillCsvStatistics(String[] problemInfo){
        ...
        problemInfo[18] = pStat.getProperty("valuation.false.occurences");
        problemInfo[19] = pStat.getProperty("valuation.false.max-depth");
        problemInfo[20] = pStat.getProperty("formula.total.variable.number");
        problemInfo[21] = pStat.getProperty("formula.total.connective.number");
        problemInfo[22] = pStat.getProperty("formula.total.variable.occurences");
        problemInfo[23] = pStat.getProperty("formula.total.variable.max-depth");
        problemInfo[24] = pStat.getProperty("formula.total.connective.occurences");
        problemInfo[25] = pStat.getProperty("formula.total.connective.max-depth");
        problemInfo[26] = pStat.getProperty("formula.negation.occurences");
        problemInfo[27] = pStat.getProperty("formula.negation.max-depth");
        problemInfo[28] = pStat.getProperty("formula.conjunction.occurences");
        problemInfo[29] = pStat.getProperty("formula.conjunction.max-depth");
        problemInfo[30] = pStat.getProperty("formula.disjunction.occurences");
        problemInfo[31] = pStat.getProperty("formula.disjunction.max-depth");
        problemInfo[32] = pStat.getProperty("formula.implication.occurences");
        problemInfo[33] = pStat.getProperty("formula.implication.max-depth");
        problemInfo[34] = pStat.getProperty("total.variable.occurences");
        problemInfo[35] = pStat.getProperty("total.variable.max-depth");
        problemInfo[36] = pStat.getProperty("total.variable.number");
        problemInfo[37] = pStat.getProperty("total.connective.occurences");
        problemInfo[38] = pStat.getProperty("total.connective.max-depth");
        problemInfo[39] = pStat.getProperty("total.connective.number");
        problemInfo[40] = pStat.getProperty("total.symbol.max-depth");
        problemInfo[41] = pStat.getProperty("total.symbol.length");
    }
    ...
    public static void parseCalculus(Set<MettelGeneralTableauRule> calculus) throws IOException, RecognitionException{
        CharStream tin = new ANTLRInputStream(LukavronBenchmark.class.getResourceAsStream("calculus"));
        tokens.setTokenSource(new LukavronLexer(tin));
